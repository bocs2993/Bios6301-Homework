...
### Notes/Grade ### 
```{r}
## Question 1: using par(mfrow=c(3,3)) will allow you to see all of the plots in a grid

## Question 2: great

## Question 3: this was actually last year's hw question, but I graded it as is :)
## Needed to set seed and also calculate the proportion of CIs that included 0. see code below
# 7/10 pts
...
std <- 2/sqrt(12)
trials <- data.frame(SampleMean = rep(NA,1000), 
                     LowerBound = rep(NA,1000), 
                     UpperBound = rep(NA,1000),
                     ContainsMean = rep(NA,1000),
                     Contains0 = rep(NA,1000)
                     )
set.seed(1000) ## need to set seed to 1000
for (i in 1:1000){
  dat <- runif(100, -1, 1)
  trials$SampleMean[i] <- mean(dat)
  confInt <- trials$SampleMean[i] + qnorm(c(0.025,0.975))*std/10
  trials$LowerBound[i] <- confInt[1]
  trials$UpperBound[i] <- confInt[2]
  ## this gives what proportion of times the sample mean is in the CI,
  ## which is going to be 100% because the CI is centered at the sample mean
  if (trials$SampleMean[i] >= confInt[1] & trials$SampleMean[i] <= confInt[2]) {
    trials$ContainsMean[i] <- 1
  }
  ## The question asks what proportion of times the CI contains ZERO:
  trials$Contains0[i] <-ifelse(0 >= confInt[1] & 0 <= confInt[2], 1, 0)
}

trials[1:10,]
mean(trials$ContainsMean) #100% of the time
mean(trials$Contains0) #95% of the time!
```
