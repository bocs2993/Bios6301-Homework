...
```
Gender appears to affect 'los' the most with a factor at ~4.5.
```
...
```{r}
stderr <- sqrt(diag(vcov(summary(mod))))
stderr
```
...
```{r}
p.val <- 2*pt(testStat[2],7,lower.tail=F)
p.val
```
...
```{r}
resStdErr <- sqrt(deviance(mod)/df.residual(mod))
resStdErr
summary(mod)$sigma
```
...
When `var.equal` is set to TRUE, the p-value equals the p-value for gender form the model summary.
